...
// CHANGES: Structure to pass arguments to thread
typedef struct{
	GtkTextBuffer *buf;
	gchar *str;
	gboolean *retval;
...
// CHANGES: (Added function) The task of thread. 
static void * par_search(void* dat)
{
    ...
    gtk_text_buffer_get_bounds(d->buf, &start, &limit);
    gint count = gtk_text_buffer_get_char_count(d->buf);
	gtk_text_buffer_get_iter_at_offset(d->buf, &iter, (count/2));
	printf("The number of characters is : %d\n", count);

	GtkTextTag *tag;
	tag = gtk_text_tag_table_lookup (d->buf->tag_table, "searched");
	printf("%s\n", tag->name);

	// iter = start;
	/* CHANGES: gtk_text_iter_forward_search searches for matches
		gtk_text_buffer_apply_tag - thread unsafe function causing seg fault.
	*/
	do {
		
		res = gtk_text_iter_forward_search(
			&iter, d->str, d->search_flags, &start, &end, &limit);
		// printf("The number of characters is : %d\n", gtk_text_buffer_get_char_count(d->buf));
		if(gtk_text_buffer_get_modified(d->buf) == TRUE)
				printf("%s\n", "FROM THREAD : MODIFIED");
		if (res) {
			*d->retval = TRUE;
			gtk_text_buffer_apply_tag(d->buf,
				tag, &start, &end);
//				replace_mode ? "replaced" : "searched", &start, &end);
			printf("%s\n", "Hello from the inside if");
			iter = end;
		}
	} while (res);

	gtk_text_buffer_get_bounds(d->buf, &start, &end);
	printf("Printinf the character : %c\n", gtk_text_buffer_get_text(d->buf, &start, &end, FALSE)[0]);
}

static gboolean hlight_searched_strings(GtkTextBuffer *buffer, gchar *str)
{
	GtkTextIter iter, start, end, limit;
	gboolean res, retval = FALSE, retval2 = FALSE;
	GtkSourceSearchFlags search_flags =
...
/*	gtk_text_buffer_remove_tag_by_name(buffer,
	...
	gtk_text_buffer_get_iter_at_offset(buffer, &limit, count/2);

	

	GtkTextTag *tag;
	tag = gtk_text_tag_table_lookup (buffer->tag_table, "searched");

	// GtkTextTagTable *	table;
	// table = buffer->

	// CHANGES: Initialising thread parameters
	...
	// CHANGES: Creating thread
	pthread_create(&tid, NULL, par_search, (void *) data);

	iter = start;
	do {
		res = gtk_text_iter_forward_search(
			&iter, str, search_flags, &start, &end, &limit);
		if (res) {
			retval = TRUE;
...
/*	if (replace_mode)
	...
	pthread_join(tid, NULL);
	// CHANGES: Never reaches here since program crashes due to seg fault.
	printf("%s\n", "Done");	
